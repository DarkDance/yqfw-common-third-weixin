...
public interface WxMpKfApiProxy {
    ...
    WeixinRspV1 kfAccountAdd(@RequestParam String access_token, @RequestBody WxMpKfAccountParam request) throws BusinessException;

    //客服管理 - 邀请绑定客服账号
    @PostExchange(url = "/customservice/kfaccount/inviteworker")
    WeixinRspV1 kfAccountInviteWorker(@RequestParam String access_token, @RequestBody WxMpKfAccountParam request) throws BusinessException;

    //客服管理 - 修改客服账号
    @PostExchange(url = "/customservice/kfaccount/update")
    WeixinRspV1 kfAccountUpdate(@RequestParam String access_token, @RequestBody WxMpKfAccountParam request) throws BusinessException;

    //客服管理 - 删除客服账号
    @PostExchange(url = "/customservice/kfaccount/del")
    WeixinRspV1 kfAccountDel(@RequestParam String access_token, @RequestBody WxMpKfAccountParam request) throws BusinessException;

    //客服管理 - 设置客服账号的头像，文件大小为5M以内
    @PostExchange(url = "/customservice/kfaccount/uploadheadimg", contentType = "multipart/form-data")
    WeixinRspV1 kfAccountUploadHeadImg(@RequestParam String access_token, @RequestParam String kf_account, @RequestParam Resource media) throws BusinessException;
    ...
    WeixinRspV1 sendKefuMessageWithResponse(@RequestParam String access_token, @RequestBody ReplyMsgData request) throws BusinessException;

    //客服消息 - 发送客服输入状态
    @PostExchange(url = "/cgi-bin/message/custom/typing")
    WeixinRspV1 sendKfTypingState(@RequestParam String access_token, @RequestBody WxMpKfTypingParam request) throws BusinessException;

    //会话控制 - 创建会话
    @PostExchange(url = "/customservice/kfsession/create")
    WeixinRspV1 kfSessionCreate(@RequestParam String access_token, @RequestBody WxMpKfSessionData request) throws BusinessException;

    //会话控制 - 关闭会话
    @PostExchange(url = "/customservice/kfsession/close")
    WeixinRspV1 kfSessionClose(@RequestParam String access_token, @RequestBody WxMpKfSessionData request) throws BusinessException;
